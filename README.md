...
└── pages/
    ...
    └── 2_Scanner.py
```
Important: the `.streamlit` folder (with the leading dot) controls the
app's colors/theme. It's easy to miss since some file managers hide
dot-folders by default — make sure it's included and named exactly
...
Watchlist dropdown at the top: choose "NIFTY 500 (all)" (default)
or any watchlist you've saved from the Scanner page.
Search box: type a symbol or company name to jump to it directly —
...
Click any stock in the left panel — the chart on the right updates
immediately.
Timeframe: Daily / Weekly / Monthly, applies to whichever stock is
selected.
On the chart itself: scroll to zoom, click-and-drag to pan,
hover a candle to see its OHLC values in the small legend above
the chart.
This page ships with no indicator overlays by design, since you're
...
Sidebar → Stock Universe: auto-downloads the NIFTY 500 list from
...
constituents").
Build rules: each rule has its own timeframe dropdown plus a
...
Click Scan NIFTY 500. The first scan takes a few minutes since it
downloads price history for up to 500 stocks — repeat scans within
the same day are faster since results are cached.
Review the shortlist, inspect any stock's chart with RSI/MACD panels,
...
Switch to the Chart page — your saved watchlist now appears in the
